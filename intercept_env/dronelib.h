...
void compute_drone_observations(Drone* agent, float* observations) {
    ...
    // Relative velocity in body frame: (v_target - v_self) rotated to body.
    // Target.vel is per-step displacement (m / ACTION_DT), so divide back to m/s.
    Vec3 v_target_mps = scalmul3(agent->target->vel, 1.0f / ACTION_DT);
    Vec3 v_rel_world = sub3(v_target_mps, agent->state.vel);
    Vec3 v_rel_body = quat_rotate(q_inv, v_rel_world);
    observations[idx++] = v_rel_body.x / denom;
    observations[idx++] = v_rel_body.y / denom;
    observations[idx++] = v_rel_body.z / denom;
