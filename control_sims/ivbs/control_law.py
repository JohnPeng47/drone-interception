...
def terminal_visual_command(
    ...
    pursuer_v_w = np.asarray(snapshot.pursuer.velocity_w, dtype=float).reshape(3)
    closing_speed = float(pursuer_v_w @ n_t)
    max_closing_speed = float(gains.get("terminal_closing_speed_max_mps", 2.5))
    closing_accel = float(gains.get("terminal_closing_accel_mps2", 1.0))
    if closing_speed > max_closing_speed:
        closing_accel = -float(gains.get("terminal_overspeed_decel_mps2", 2.0))
    bearing_error = float(np.arccos(np.clip(float(n_td @ n_t), -1.0, 1.0)))
    cutoff = max(float(gains.get("terminal_bearing_thrust_cutoff_rad", 0.55)), 1.0e-6)
    thrust_scale = float(np.clip(1.0 - bearing_error / cutoff, 0.0, 1.0))
    damping = float(gains.get("terminal_velocity_damping", 0.55))
    a_d = thrust_scale * closing_accel * n_t - damping * pursuer_v_w
