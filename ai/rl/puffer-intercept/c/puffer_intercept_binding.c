...
static void reset_env_slot(Env* env) {
    ...
    env->previous_distance_m = env->engine.metrics.distance_m;
    float max_rate = scenario->max_rate_rps > 0.0f ? scenario->max_rate_rps : scenario->pursuer_params.max_omega;
    write_observation(&env->engine, max_rate, scenario->pursuer_params.max_rpm, env->observations);
}
...
void my_init(Env* env, Dict* kwargs) {
    ...
    env->cursor = 0;
    env->reward = (NativeRewardConfig){
        .catch_reward = 10.0f,
        .distance_weight = 0.001f,
        .progress_weight = 0.1f,
        .fail_penalty = 30.0f,
        .rate_weight = 2e-4f,
    };
}
...
void c_step(Env* env) {
    ...
    float reward =
        (intercepted ? env->reward.catch_reward : 0.0f) +
        env->reward.progress_weight * (env->previous_distance_m - distance) -
        env->reward.distance_weight * distance -
        env->reward.rate_weight * rate_norm -
        (failed ? env->reward.fail_penalty : 0.0f);
    env->previous_distance_m = distance;
