...
class PufferPPO:
    ...
    def update(
        ...
        *,
        ...
        for _ in range(num_minibatches):
            ...
            if collect_diagnostics:
                ...
                _append_diagnostic_sample(
                    ...
                    diagnostic_counts,
                    "td_error",
                    _td_error(mb_values, rew[idx], ter[idx], cfg.gamma),
                    diagnostic_sample_size,
                )
                _append_paired_diagnostic_sample(
                    diagnostics,
                    diagnostic_counts,
                    "value_return",
                    newvalue.view(mb_returns.shape),
                    mb_returns,
                    diagnostic_sample_size,
                )
        ...
        return out, _finalize_diagnostics(diagnostics)
...
def _td_error(values: torch.Tensor, rewards: torch.Tensor, dones: torch.Tensor, gamma: float) -> torch.Tensor:
    td = torch.zeros_like(values)
    if values.shape[1] <= 1:
        return td
    next_nonterminal = 1.0 - dones[:, 1:]
    td[:, :-1] = rewards[:, 1:] + float(gamma) * values[:, 1:] * next_nonterminal - values[:, :-1]
    return td
...
def _append_diagnostic_sample(
    ...
    remaining = int(max_samples) - int(counts[key])
    if remaining <= 0:
        return
    ...
    if take < flat.numel():
        indices = torch.randperm(flat.numel(), device=flat.device)[:take]
        flat = flat[indices]
    diagnostics[key].append(flat.float().cpu())
    counts[key] += int(take)


def _append_paired_diagnostic_sample(
    diagnostics: dict[str, list[torch.Tensor]],
    counts: dict[str, int],
    key: str,
    first: torch.Tensor,
    ...
    count = min(first_flat.numel(), second_flat.numel())
    if count == 0:
        return
    take = min(remaining, count)
    if take < count:
        indices = torch.randperm(count, device=first_flat.device)[:take]
        first_flat = first_flat[indices]
        second_flat = second_flat[indices]
    else:
        first_flat = first_flat[:take]
        second_flat = second_flat[:take]
    diagnostics[key].append(torch.stack((first_flat.float().cpu(), second_flat.float().cpu()), dim=1))
    counts[key] += int(take)


def _finalize_diagnostics(diagnostics: dict[str, list[torch.Tensor]]) -> dict[str, np.ndarray]:
    out = {}
    for key, chunks in diagnostics.items():
        if not chunks:
            continue
        out[key] = torch.cat(chunks, dim=0).numpy()
    return out
