...
from intercept_sim.types import SimulationTarget
...
class KinematicTarget:
    ...
    def state_at(self, t: float) -> SimulationTarget:
        return SimulationTarget(
            id=self.target_id,
            kind=self.kind,
            position_w=np.asarray(self.initial_position_w, dtype=float) + float(t) * np.asarray(self.velocity_w, dtype=float),
